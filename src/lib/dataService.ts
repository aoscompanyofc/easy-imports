...
export const dataService = {
  ...
  async addSale(sale: any, items: any[]) {
    ...
    const p1 = { ...base, sale_number, sale_type, installments: inst, sign_token, pdf_type,
      ...
      seller_id: rep_seller_id, seller_display_name: rep_seller_name, incoming_devices_json, installments_json, outgoing_items_json };
    ...
    const p2 = { ...base, sale_number, sale_type, installments: inst, sign_token, pdf_type,
      ...
      incoming_name, incoming_imei, incoming_serial, incoming_email,
      incoming_capacity, incoming_color, incoming_condition, incoming_battery_health, incoming_purchase_price,
      installments_json, outgoing_items_json };
    ...
    const p5 = { ...base, sale_number, sale_type, installments: inst, sign_token, installments_json, outgoing_items_json };
  ...
  // Mesmo padrão do Dashboard Layout / Tarefas: um blob JSON por usuário em
  // Supabase, com fallback em localStorage caso a tabela ainda não exista.
  async getProcessosBoard(): Promise<{ processes: any[]; goals: any[] } | null> {
    const local = () => {
      try {
        const raw = localStorage.getItem('easy-imports-processos-board');
        return raw ? JSON.parse(raw) : null;
      } catch { return null; }
    };
    if (useMock) return local();
    try {
      ...
      if (data?.data) return data.data as { processes: any[]; goals: any[] };
      return local();
    } catch {
      return local();
    }
  },
  async saveProcessosBoard(board: { processes: any[]; goals: any[] }): Promise<boolean> {
    try {
      localStorage.setItem('easy-imports-processos-board', JSON.stringify(board));
    } catch { /* ignore */ }
    if (useMock) return true;
    try {
      const uid = await getUid();
